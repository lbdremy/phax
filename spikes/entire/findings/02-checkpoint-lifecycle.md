...
| 5   | Merge and publish               | **Publish: safe.** `git ls-remote origin 'refs/entire/*'` is empty after the run auto-published; `refs/entire/*` sits outside `refs/heads/*` so a normal push does not carry it. **Merge: observed 2026-08-18** (PR #82, rebase merge). All five phase commits replayed onto `main` with new shas; all trailers survived and trailer → ref resolved 5/5; the shas did not survive. Squash-merge trailer collapse still unobserved — a rebase merge keeps one trailer set per commit. |
...
2. **Does it survive merge and publish intact? Yes to both — merge confirmed
   2026-08-18.** A rebase merge preserved every trailer and every trailer → ref
   resolution, while replacing the shas: the record survives, sha-keyed addressing
   does not. That finding overturned the record-keying decision in
   `docs/spikes/entire-checkpoint-findings.md`. Original pre-merge wording follows.
   The
