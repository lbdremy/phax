...
> **Draft, not installed.** Ground for the `review-as-plan` spec
> (`../headless-code-review.md`) and the `review-plan` command. Distilled 2026-09-24 from
> the steme roadmap-1.0 protocol §5.6, §12.3 and decision P4. A project extends it with
> `--doctrine <file>` and sets its threshold with `--min-severity`.
