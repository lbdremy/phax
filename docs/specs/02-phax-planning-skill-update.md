...
`## What the next phase needs to know`. phax flags *what* deviated; only the
agent knows *why*. See `.skills/phax-phase-handoff.md` for the per-section
guidance.
...
the naming nuance: `optionalFilesToEdit` describes files whose *contents* are
optional to touch; the *field itself* is still required.
