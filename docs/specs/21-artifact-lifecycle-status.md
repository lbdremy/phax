...
The lifecycle exists in people's heads and in skill prose, not in the system.
...
triggers without changing the vocabulary.
...
The system SHALL accept exactly the state set `Draft, Approved, Archived` for specs.

The system SHALL accept exactly the state set `Draft, Approved, Stale, Archived` for plans.
...
- Spec: `Draft → Approved`, `Approved → Archived`.
- Plan: `Draft → Approved`, `Approved → Approved` (re-approval — a status no-op that
  refreshes the approval record; record semantics belong to the staleness & lineage spec),
  `Approved → Stale`, `Approved → Archived`, `Stale → Approved` (false-positive override:
  a human reviewed the stale verdict and the plan still holds), `Stale → Draft`,
  `Stale → Archived`.
...
IF an artifact's status is `Archived` and the file is outside its archive location, or the
file is inside its archive location with a status other than `Archived`, THEN validation
of that artifact SHALL fail naming the disagreement.

WHEN a transition to `Archived` is applied through phax THE system SHALL move the artifact
file to its archive location as part of the transition.
...
(`Draft`, `Approved`, `Stale`, `Archived`) **normative**; position within the header
region **indicative**:
...
validation fails naming the allowed set `Draft, Approved, Archived`. (refs §5.2)
...
refused and the response names `Draft → Approved` as the only legal transition. (refs §5.3)
...
`plan`, the status `Approved`, and the legal transitions `Approved`, `Stale`, `Archived`.
(refs §5.6)
...
Settled: both state sets (including `Stale` for plans — the vocabulary is fixed here and
the staleness & lineage spec must not extend it), the legal-transition tables, run gating
on `Approved` with a distinct `Stale` refusal, archive-location agreement, and in-file
status per the §9 default.
...
landed, source spec edited) and the lineage records that make it computable — nothing in
this spec should preclude that trigger being system-initiated rather than operator-typed.
