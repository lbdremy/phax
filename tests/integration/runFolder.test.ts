...
describe("createPhaseFolder", () => {
  it("creates a status.json with state: pending", async () => {
    ...
    await Effect.runPromise(createPhaseFolder(runPath, phase, 0, "feature/my-run--phase-01" as BranchName).pipe(Effect.provide(layer)));
  ...
  it("returns the phase path", async () => {
    ...
    const phasePath = await Effect.runPromise(
      createPhaseFolder(runPath, phase, 0, "feature/my-run--phase-01" as BranchName).pipe(Effect.provide(layer)),
    );
