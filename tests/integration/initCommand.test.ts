...
describe("runInit --yes (non-interactive)", () => {
  it("creates phax.json, phax.schema.json, and phax.user.schema.json from package.json", async () => {
    ...
    try {
      ...
      const fastCommands = gateProfiles["fast"] as Array<{ command: string }>;
      expect(fastCommands.some((s) => s.command.includes("typecheck"))).toBe(true);
      expect(fastCommands.some((s) => s.command.includes("lint"))).toBe(true);
      expect(fastCommands.some((s) => s.command.includes("test:unit"))).toBe(true);
