...
// Each case cold-starts `tsx` in a subprocess (some twice). A single run is
// sub-second, but under the fully-parallel `vitest run` the default 5s budget
// is too tight for two cold-starts on a loaded machine — give them headroom.
const CLI_SPAWN_TIMEOUT_MS = 30_000;

describe("phax --usage", () => {
  it(
    "prints the KDL spec to stdout and exits 0",
    () => {
  ...
  it(
    "KDL output matches the committed phax.usage.kdl byte-for-byte",
    () => {
      const result = runCli(["--usage"]);
      expect(result.status, `stderr: ${result.stderr}`).toBe(0);
  ...
  it(
    "--usage-format kdl is equivalent to the default",
    () => {
  ...
  it(
    "--usage-format json produces JSON output when usage CLI is available",
    () => {
  ...
  it(
    "--usage-format with invalid value exits non-zero with an actionable error",
    () => {
  ...
  it(
    "works when passed before a subcommand (preAction hook intercepts)",
    () => {
