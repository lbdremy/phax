...
describe("executePlan — per-phase branch regression", () => {
  ...
  it("creates two addWorktree calls with distinct branches and correct createBranch chain", async () => {
    ...
    const worktreeBranches = addWorktreeCalls.map((c) => (c as { method: "addWorktree"; branch: string }).branch);
    ...
    const createBranchCalls = fakeGit.impl.calls.filter((c) => c.method === "createBranch") as Array<{
      method: "createBranch";
      branch: string;
      from: string;
    }>;
